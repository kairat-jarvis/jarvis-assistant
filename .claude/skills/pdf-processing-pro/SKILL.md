...
### OCR Processing — three-tier waterfall

Стандарт JARVIS: `pdfplumber → PaddleOCR-VL-1.5 → Claude Vision (fallback)`.
Порядок фиксирован, пропуск уровней запрещён. Tesseract/EasyOCR/doctr/surya
**запрещены политикой проекта**.

- Tier 1 — текстовый слой
- Tier 2 — PaddleOCR-VL-1.5 (единственный допустимый OCR-движок)
- Tier 3 — Claude Vision (только если tier 2 не справился; каждая эскалация
  логируется в `jarvis_agent_logs`)
...
```bash
pip install pdfplumber pypdf pillow pandas pdf2image
```

Waterfall OCR (tiers 2 & 3):
```bash
# Tier 2 — PaddleOCR-VL-1.5 (единственный допустимый OCR-движок)
...
# poppler (для pdf2image): brew install poppler | apt-get install poppler-utils
# на Windows — скачать бинарник и задать POPPLER_PATH
```
...
**PaddleOCR не установлен (уровень 2 waterfall недоступен)**:
```bash
pip install 'paddleocr>=3.1' paddlepaddle numpy
# PaddleOCR-VL-1.5 — единственный допустимый OCR-движок в проекте.
# НЕ устанавливать tesseract/easyocr/doctr/surya как замену — они запрещены политикой.
```

**Claude Vision fallback не работает**:
```bash
# Проверьте, что anthropic SDK установлен и задан ANTHROPIC_API_KEY
pip install 'anthropic>=0.40'
export ANTHROPIC_API_KEY=sk-ant-...
```
